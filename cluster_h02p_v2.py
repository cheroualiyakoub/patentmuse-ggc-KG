...
on a WEIGHTED multi-view graph combining CITES_PATENT and SIMILAR_TITLE edges.

v2 adds the SIMILAR_TITLE signal to v1's citation-only clustering. Expected effects:
  - 15,783 patents that were singletons in v1 should now join real clusters
  - Cluster structure may shift as title-semantic evidence reinforces or revises
    citation-derived boundaries
  - Modularity may drop numerically (denser graph) but should reflect deeper structure

Writes results to local JSON; does NOT mutate Neo4j.

Run:
    /home/yacoubcherouali/kg-builder/venv/bin/python cluster_h02p_v2.py
"""
...
OUT_DIR = SCRIPT_DIR / "cluster_h02p_v2_out"
...
# Edge weight scheme — calibrated to keep citations as the dominant signal even
# though there are ~6x more title edges than citation edges per node.
# A typical node has ~5 citations × 3.0 = 15 citation weight
#               vs.   ~30 title edges × 0.4 (avg score 0.85 × 0.5) = 12 title weight
# Roughly balanced, with citation taking slight edge per-incident.
CITE_WEIGHT = 3.0
TITLE_WEIGHT_FACTOR = 0.5    # final weight = score × factor
...
def fetch_representatives(session):
    ...
    RETURN f.family_id        AS family_id,
           ...
           assignees
    """
    print(f"[1/5] Fetching family representatives for {SUBCLASS} {COUNTRIES}...")
    t0 = time.time()
...
def fetch_citations(session, pubs):
    ...
    """
    print(f"[2/5] Fetching citation edges for {len(pubs):,} reps...")
    edges = []
    t0 = time.time()
    ...
    print(f"      {len(edges):,} raw outgoing citation edges in {time.time()-t0:.1f}s")
    return edges


def fetch_similar_title(session, pubs):
    """Fetch SIMILAR_TITLE edges from each rep. Direction-agnostic (Cypher
    pattern matches both directions). Score becomes part of the edge weight."""
    query = """
    UNWIND $batch AS source_pub
    MATCH (s:Patent {publication_number: source_pub})-[r:SIMILAR_TITLE]-(t:Patent)
    RETURN source_pub AS source, t.publication_number AS target, r.score AS score
    """
    print(f"[3/5] Fetching SIMILAR_TITLE edges for {len(pubs):,} reps...")
    edges = []
    t0 = time.time()
    for i in tqdm(range(0, len(pubs), BATCH_SIZE), desc="title batches"):
        batch = pubs[i : i + BATCH_SIZE]
        for r in session.run(query, batch=batch):
            edges.append((r["source"], r["target"], r["score"] if r["score"] is not None else 0.8))
    print(f"      {len(edges):,} raw SIMILAR_TITLE edges in {time.time()-t0:.1f}s")
    return edges
...
def fetch_ipc_per_rep(session, pubs):
    ...
    """
    print(f"[4/5] Fetching IPC groups for {len(pubs):,} reps...")
    out = {}
    t0 = time.time()
...
# ------------------------------------------------------------------------------
def build_graph(reps, cite_edges, title_edges):
    """Build undirected weighted igraph from in-scope citations + SIMILAR_TITLE.
    Pairs with both signals get their weights summed."""
    pub_to_idx = {p["pub"]: i for i, p in enumerate(reps)}

    edge_weight = defaultdict(float)   # (a,b) sorted → weight
    ...
    print(f"      Citation edges in-scope: {n_cite_in_scope:,}")
    print(f"      SIMILAR_TITLE edges in-scope: {n_title_in_scope:,}")
...
def run_leiden(g, resolutions):
    print("[5/5] Running weighted Leiden at multiple resolutions...")
    out = {}
...
def main():
    ...
    print("\n[graph] Building weighted multi-view igraph...")
    g, pub_to_idx = build_graph(reps, cite_edges, title_edges)
